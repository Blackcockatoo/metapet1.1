...
export function EyeRenderer({
  ...
  // Calculate jitter offset for scared emotion — use offset as deterministic seed
  // so rendering stays pure (parent drives re-renders for animation)
  const jitterSeed = (offset?.x ?? 0) * 1000 + (offset?.y ?? 0) * 317;
  const jitterOffset = jitter
    ? {
        x: (Math.sin(jitterSeed * 0.1) * 0.5) * 1.5,
        y: (Math.cos(jitterSeed * 0.13) * 0.5) * 1.5,
      }
    : { x: 0, y: 0 };
  ...
  const renderEye = (center: { x: number; y: number }, isLeft: boolean) => {
    ...
    const pupilColor = eyeState.pupilColor || '#ffd700';
    ...
    return (
      ...
      >
        ...
        {shape === 'slit' ? (
          ...
          <g>
            <circle cx={finalX} cy={finalY} r={pupilSize} fill={pupilColor} opacity="0.7" />
            <path
          ...
          // Round pupil (default)
          <circle cx={finalX} cy={finalY} r={pupilSize} fill={pupilColor} opacity="0.6" />
        )}
