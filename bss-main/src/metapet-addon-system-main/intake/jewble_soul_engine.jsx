...
function pushParticle(store: Particle[], particle: Particle) {
  if (particle.role === "spark") {
    let sparkCount = 0;
    for (let i = 0; i < store.length; i++) if (store[i].role === "spark") sparkCount++;
...
export default function JewbleSoulEngine() {
  ...
  const triggerCoronationBurst = useCallback(
    (power: number, hue: number) => {
      ...
      if (shockwavesRef.current.length > 10) shockwavesRef.current.splice(0, shockwavesRef.current.length - 10);
  ...
  const disableAudioReactive = useCallback(async () => {
    ...
    try {
      ...
      if (refs.context && refs.context.state !== "closed") await refs.context.close();
    } catch {
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      ...
      const dt = clamp((time - lastTRef.current) / 16.667, 0.55, 2.1);
      ...
      const blinkWindow =
        smoothstep(0, 8, blinkRef.current.timer) * (1 - smoothstep(8, 18, blinkRef.current.timer));
      blinkRef.current.amount =
        audioLevel > 0.24 ? Math.max(0, 1 - audioLevel * 0.4) : 1 - blinkWindow;
      const eyeOpen = clamp(blinkRef.current.amount, 0.08, 1);

      ctx.fillStyle = `rgba(2,4,18,${0.17 - audioLevel * 0.04 - beatFlash * 0.03})`;
      ...
      for (let ring = 4; ring >= 0; ring--) {
        ...
        const twist =
          frameRef.current * (0.0036 + audioLevel * 0.002 + beatFlash * 0.0012) *
            (ring % 2 === 0 ? 1 : -1) +
          ring * 0.36;
        ...
        if (ring === 2) {
          for (let i = 0; i < pts.length; i++) {
            ...
            const gp =
              0.7 +
              0.45 * Math.sin(frameRef.current * 0.07 + i * 0.9) +
              audioLevel * 0.8 +
              beatFlash * 0.9;
            ctx.beginPath();
      ...
      // ── Stronger helix body + mood morphology ─────────────────────────────
      for (let s = 0; s < 7; s++) {
        const strandHue = (em.hue + s * 42) % 360;
        const baseAngle = (TAU * s) / 7;
        const offsetWave =
          Math.sin(helixTime * 1.2 + s * 0.8) * R * (0.035 + audioLevel * 0.02 + beatFlash * 0.02);
        ...
        for (let n = 0; n < 9; n++) {
          ...
          const glyph =
            GLYPHS[(n + s + Math.floor(frameRef.current * (0.08 + audioLevel * 0.18 + beatFlash * 0.16))) % GLYPHS.length];
          const flicker =
            0.16 +
      ...
      // ── Core jewel eye with eye states ────────────────────────────────────
      ...
      const irisRx = coreR * (1.04 + audioLevel * 0.08 + beatFlash * 0.05);
      const irisRy = coreR * (0.92 + eyeOpen * 0.24 - audioLevel * 0.06 + beatFlash * 0.03);
      const pupilRx = coreR * (0.28 + (1 - eyeOpen) * 0.06 + audioLevel * 0.11 + beatFlash * 0.05);
      const pupilRy = coreR * (0.36 * eyeOpen + 0.08 + (em.name === "ALERT" ? 0.06 : 0));

      ctx.beginPath();
      ctx.arc(cx, cy, coreR * 2.7, 0, TAU);
      const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, coreR * 2.7);
      grad.addColorStop(0, `hsla(${em.hue}, 100%, 78%, ${(0.36 + audioLevel * 0.12 + beatFlash * 0.18) * tBlend})`);
      grad.addColorStop(0.45, `hsla(${(em.hue + 34) % 360}, 84%, 56%, ${(0.18 + audioLevel * 0.08 + beatFlash * 0.14) * tBlend})`);
      ...
      ctx.fillStyle = `hsla(${em.hue}, 100%, 90%, ${0.94 * tBlend})`;
      ctx.shadowColor = `hsla(${em.hue}, 100%, 70%, 0.95)`;
      ctx.shadowBlur = coreGlow;
      ctx.fill();
      ctx.shadowBlur = 0;

      const irisGrad = ctx.createRadialGradient(0, -coreR * 0.14, coreR * 0.08, 0, 0, coreR * 1.1);
      irisGrad.addColorStop(0, `hsla(${(em.hue + 28) % 360}, 100%, 95%, ${0.86 + audioLevel * 0.08 + beatFlash * 0.08})`);
      irisGrad.addColorStop(0.5, `hsla(${em.hue}, 100%, 70%, ${0.82 + audioLevel * 0.08 + beatFlash * 0.08})`);
      irisGrad.addColorStop(1, `hsla(${(em.hue + 180) % 360}, 100%, 30%, ${0.72 + beatFlash * 0.1})`);
      ctx.beginPath();
      ctx.ellipse(0, 0, irisRx * 0.82, irisRy * 0.82, 0, 0, TAU);
      ctx.fillStyle = irisGrad;
      ctx.fill();

      ctx.beginPath();
      ctx.ellipse(0, 0, pupilRx, pupilRy, 0, 0, TAU);
      ctx.fillStyle = "rgb(2,4,18)";
      ctx.fill();

      ctx.beginPath();
      ctx.ellipse(-coreR * 0.16, -coreR * 0.18, coreR * 0.18, coreR * 0.1, -0.45, 0, TAU);
      ctx.fillStyle = `rgba(255,255,255,${0.18 + audioLevel * 0.18 + beatFlash * 0.18})`;
      ctx.fill();

      ctx.rotate(frameRef.current * (0.04 + audioLevel * 0.05 + beatFlash * 0.03));
      ctx.strokeStyle = `hsla(${(em.hue + 180) % 360}, 100%, 82%, ${(0.74 + audioLevel * 0.18 + beatFlash * 0.18) * tBlend})`;
      ctx.lineWidth = 1 + audioLevel * 0.8 + beatFlash * 0.9;
      for (let i = 0; i < 7; i++) {
        const a = (TAU * i) / 7;
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.lineTo(
          Math.cos(a) * coreR * (0.42 + audioLevel * 0.06 + beatFlash * 0.08),
          Math.sin(a) * coreR * (0.42 + audioLevel * 0.06 + beatFlash * 0.08)
        );
        ctx.stroke();
      }
      ctx.restore();

      // ── Bass-triggered crown geometry / coronation bursts ─────────────────
      ...
      // ── Outer genome ring + binary/hex pulse glyphs ───────────────────────
      ...
      for (let i = 0; i < 60; i++) {
        ...
        if (i % 5 === 0) {
          ...
          const flicker = 0.12 + 0.28 * Math.abs(Math.sin(frameRef.current * (0.11 + audioLevel * 0.12 + beatFlash * 0.1) + i)) + audioLevel * 0.22 + beatFlash * 0.18;
      ...
      // ── Singularities: black-hole click burst ──────────────────────────────
      ...
      for (let i = parts.length - 1; i >= 0; i--) {
        ...
        if (p.role === "seeker") {
          ...
          const strandAngle =
            (TAU * p.strand) / 7 +
            frameRef.current * (0.015 + audioLevel * 0.012 + beatFlash * 0.008) +
            p.helixPhase * 0.2;
      ...
      if (seekerCount < MAX_SEEKERS) spawn(audioLevel > 0.16 || beatFlash > 0.1 ? 5 : 4);
      ...
      // ── CSS-free ripples drawn on canvas ───────────────────────────────────
  ...
  return (
    ...
    >
        ...
        {EMOTIONS.map((e, i) => (
          <button
            ...
            style={{
              ...
              border:
                i === emotion
                  ? `2px solid hsla(${e.hue},100%,70%,0.9)`
                  : `1px solid hsla(${e.hue},60%,50%,0.3)`,
      ...
      >
        ...
        <div style={{ color: `hsla(${em.hue},100%,70%,0.9)` }}>
          STRAND {(emotion + 1).toString().padStart(2, "0")} / 07
        </div>
        <div>SOUL ENGINE v5</div>
